...
class GitaddMod(loader.Module):
    ...
    async def gitaddcmd(self, message): 
        if self.config["GH_TOKEN"] == "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":
            await utils.answer(message, self.strings("token_not_found", message))
            return
        if self.config["GH_USERNAME"] == "kyKversion2":
            await utils.answer(message, self.strings("username_not_found", message))
            return
        if self.config["GH_REPO"] == "Modules-kyk":
        ...
        try:
            ...
            if int(r.status_code) == 201:
                ...
                uploaded_to_raw = uploaded_to + f'/mine/{fname}'
